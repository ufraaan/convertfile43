...
enum BundlePaths {
    ...
    static var ffconv: String {
        findBinary("ffconv")
    }

    static var magick: String {
        findBinary("magick")
    }

    static var potrace: String {
        findBinary("potrace")
    }

    static var isFFConvAvailable: Bool {
        let path = ffconv
        return path != "ffconv" && FileManager.default.isExecutableFile(atPath: path)
    }
